...
MObject MG_trigonometry::inverseA;
...
MStatus MG_trigonometry::initialize()
	{ 
		...
		inverseA =numFn.create("inverseFunction","if",MFnNumericData::kBoolean);
		numFn.setKeyable(true);
		numFn.setStorable(true);
		addAttribute(inverseA);
		...
		attributeAffects(inverseA,outputValue);
...
MStatus MG_trigonometry::compute(const MPlug& plug,MDataBlock& dataBlock)
	{
		...
		{	
			...
			bool inverseV = dataBlock.inputValue(inverseA).asBool();
